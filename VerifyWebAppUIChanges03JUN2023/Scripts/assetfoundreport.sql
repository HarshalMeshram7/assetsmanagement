DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_assetfound`(In companyid int,IN batchid int)
BEGIN
 ...
 insert into temp_report_Assetfound (assetid,companyid,
		...
		update temp_report_Assetfound set Location = (select (A.ALocationName) from tblalocation A inner join tblassets D on A.id=D.LocAID where D.id=assetid) 
                where assetid > 0;
           update temp_report_Assetfound set Sublocation =(select (A.BLocationName) from tblblocation A inner join tblassets D on A.id=D.LocBID where D.id=assetid) 
                where assetid > 0; 
           update temp_report_Assetfound set Sub_Sublocation =(select (A.CLocationName) from tblclocation A inner join tblassets D on A.id=D.LocCID where D.id=assetid) 
                where assetid > 0;
                
                select * from temp_report_Assetfound;
