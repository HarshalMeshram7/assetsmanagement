...
DROP PROCEDURE IF EXISTS usp_report_FAR;
...
/*				 update temp_report_working set v_OpDep = v_OpDep - v_disposalopdep  where v_assetid > 0; -- Mandar 21 FEB 2021 */ 
	...
	/* Mandar update disposed qty 16 FEB 2021 */
		...
		/* Disposal Dep to be reversed Amount For the Period */
        	...
        	/*select * from temp_report_final where assetid =593;*/
			
            /*
			update temp_report_final  set OpDep =0 where  FLOOR(OpGross) = 0 and assetid  > 0; 
            Commentd by Mandar 05 MAY 2021
			*/	
